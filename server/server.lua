...
AddEventHandler('d-phone:server:changewallpaper', function(source, url, notif)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    MySQL.Async.fetchAll("SELECT * FROM `phone_information` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        if result[1] ~= nil then
            ...
            if not notif then
                TriggerClientEvent("d-notification", _source,  _U("changedbg"), 5000, 'rgb(212, 222, 0)')
            end
        else
            MySQL.Async.execute("INSERT INTO `phone_information` (`identifier`, `wallpaper`) VALUES (@identifier, @wallpaper)",
...
AddEventHandler('d-phone:server:changedarkmode', function(source, state)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `phone_information` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent("d-notification", _source,  _U("changeddarkmode"), 5000, 'rgb(212, 222, 0)')
            if result[1].wallpaper == Config.backgroundurl or result[1].wallpaper == Config.darkbackgroundurl or result[1].wallpaper == nil then
                if state == 1 then
                    TriggerEvent("d-phone:server:changewallpaper", _source, Config.darkbackgroundurl, true)
                else
                    TriggerEvent("d-phone:server:changewallpaper", _source, Config.backgroundurl, true)
                end
            end
        else
